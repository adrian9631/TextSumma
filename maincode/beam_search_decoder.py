  ...
  def __init__(self,
    ...
    if callable(embedding):
      self._embedding_fn = embedding
        ...
        def multi_embedding(ids, batch_size):
            embeddings = []
            for batch_id in range(batch_size):
                embeddings.append(embedding_ops.embedding_lookup(embedding[batch_id:batch_id+1], ids[batch_id:batch_id+1]))
            return array_ops.stack(embeddings, axis=0)

        self._embedding_fn = multi_embedding
     # self._embedding_fn = (
    ...
    self._start_inputs = embedding_ops.embedding_lookup(embedding[0:1], self._start_tokens)
  ...
  def step(self, time, inputs, state, name=None):
    ...
    with ops.name_scope(name, "BeamSearchDecoderStep", (time, inputs, state)):
      ...
      next_inputs = control_flow_ops.cond(
          ...
          lambda: self._embedding_fn(sample_ids, self._batch_size))
